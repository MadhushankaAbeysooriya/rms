                        ...
                        <form role="form" method="POST" action="{{route('items.save_alternative',$item->id)}}"
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group row">
                                    ...
                                    <label for="alternative_item_id" class="col-sm-2 col-form-label">Alternative Item</label>
                                    <div class="col-sm-6">
                                        <select class="form-control" name="alternative_item_id" id="alternative_item_id" autocomplete="off">
                                            <option value="" selected>select one</option>
                                            @foreach($items as $item)
                                                <option value="{{$item->id}}">{{$item->name}}</option>
                                            @endforeach
                                        </select>
                                        <span class="text-danger">@error('alternative_item_id') {{ $message }} @enderror</span>
                                    </div>
                                </div>
